...
import {setUsers, setUsernames, checkAll} from "../redux/actions";
...
const Table = () => {
    ...
    const {users, userNames} = useSelector(state => state.users)
    ...
    return (
        <div className="container mx-auto py-6 px-4">
            ...
            <h3 className="text-2xl py-4">Пользователи: <span className="text-xl">{userNames.length > 0 ? userNames.join(', ') : 'Выделите пользователей'}</span></h3>
        </div>
    )
}
